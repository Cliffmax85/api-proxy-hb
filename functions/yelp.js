...
exports.handler = async (event, context) => {
  try {
    ...
    const response = await fetch(`https://api.yelp.com/v3/businesses/search?location="${event.queryStringParameters.city},${event.queryStringParameters.state},${event.queryStringParameters.country}"`, {
      header : {
    ...
    // this is where you shoot data back to the user. right now it's sending an empty object--replace this with the yelp data. remember, you do need to stringify it, otherwise netlify gets mad. ¯\_(ツ)_/¯
      body: JSON.stringify(json.bussinesses),
    };
  } catch (error) {
    console.log(error);
    return {
